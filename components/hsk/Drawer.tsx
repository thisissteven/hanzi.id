...
function DrawerContent({ children, className, ...props }: DrawerContentProps) {
  return (
    <VaulDrawer.Portal>
      <VaulDrawer.Overlay className="fixed z-40 inset-0 bg-black/50 brightness-0" />
      <VaulDrawer.Content
        className={cn(
          "p-3 pt-4 focus:outline-none z-40 bg-black text-light-smokewhite rounded-t-[10px] fixed bottom-0 right-0",
          className
        )}
        {...props}
      >
