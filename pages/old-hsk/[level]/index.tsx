...
import { CHARACTERS_PER_PAGE_OLD, ChineseCharacter, HSK_LEVELS_OLD, LevelOld } from "@/data";
...
export const getStaticPaths = (async ({ locales }) => {
  ...
  return {
    paths: HSK_LEVELS_OLD.map((level) => {
...
const getFilePath = (level: string | number, locale?: string) =>
  `/data/${locale === "id" ? "id_old" : "en_old"}/hsk-level-${level}.json`;
...
export const getStaticProps = async ({ params, locale }: GetStaticPropsContext) => {
  ...
  const totalPages = Math.ceil(allCharacters.length / CHARACTERS_PER_PAGE_OLD);
  const previousLevelTotalPages = Math.ceil(allPreviousLevelCharacters.length / CHARACTERS_PER_PAGE_OLD);
  const currentLevel = parseInt(level) as LevelOld;
...
function usePagination({
  ...
  const characters = React.useMemo(() => {
    return allCharacters.slice(
      (parseInt(currentPage) - 1) * CHARACTERS_PER_PAGE_OLD,
      parseInt(currentPage) * CHARACTERS_PER_PAGE_OLD
    );
  ...
  const previousHref = canPreviousLevel
    ? `/old-hsk/${currentLevel - 1}?page=${previousLevelTotalPages}`
    : `/old-hsk/${currentLevel}?page=${parseInt(currentPage) - 1}`;

  const nextHref = canNextLevel
    ? `/old-hsk/${currentLevel + 1}`
    : `/old-hsk/${currentLevel}?page=${parseInt(currentPage) + 1}`;
...
export default function Page(props: InferGetStaticPropsType<typeof getStaticProps>) {
  ...
  const title = `Old HSK ${props.currentLevel} Page ${currentPage}`;
  ...
  } = useCompletedCharactersActionsOld();
            ...
            {characters.map((character) => {
              ...
              if (width > 640) {
                return (
                  <CharacterCardOld
                    ...
                    hanziHref={`/old-hsk/${props.currentLevel}/?hanzi=${character.hanzi}&id=${character.id}&page=${currentPage}`}
              ...
              return (
                <CharacterRow
                  ...
                  onClick={() =>
                    router.push(
                      `/old-hsk/${props.currentLevel}/?hanzi=${character.hanzi}&id=${character.id}&page=${currentPage}`,
                      undefined,
                      {
                        shallow: true,
                      }
